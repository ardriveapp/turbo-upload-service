...
export function assembleBundlePayload(
  ...
  logger.debug("Offsets sentinel task started");
  // Centralized cleanup for both happy and error paths
  outputStream.on("end", () => {
    coordinator.removeAllListeners();
    cleanupActiveDataItemStreams();
    logger.debug("Output stream ended; finishing offsets sentinel task...");
    try {
      offsetsTaskCounter.finishTask();
    } catch (error) {
      logger.debug("Sentinel finishTask ignored (already zero?)", { error });
    }
  });
  outputStream.on("error", (error) => {
    ...
    coordinator.removeAllListeners();
    cleanupActiveDataItemStreams();
    logger.debug("Output stream errored; finishing offsets sentinel task...");
    try {
      offsetsTaskCounter.finishTask();
    } catch (error) {
      logger.debug("Sentinel finishTask ignored (already zero?)", { error });
  ...
  const dataItemAttributesPromise = new Promise<DataItemAttributes[]>(
    (resolve) => {
      offsetsTaskCounter
        ...
        .catch((error) => {
          logger.error(
            `Error waiting for offsets task counter to finish: ${error}`
          );
