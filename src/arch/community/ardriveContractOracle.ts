...
export class ArDriveContractOracle implements ContractOracle {
  ...
  async readContract(txId: TransactionId): Promise<unknown> {
    ...
    while (!contract) {
      try {
        ...
        contract = await this.contractReaders[
          currentContractReader
        ].readContract(txId);
      } catch (error) {
        logger.warn(`Contract could not be fetched: ${error}`);
        ...
        if (readContractAttempts >= maxReadContractAttempts) {
          ...
          logger.warn("Falling back to next contract reader..");
        } else {
          logger.warn("Retrying with current contract reader..");
        }
      }
    }
    return contract;
  ...
  public async getCommunityContract(): Promise<CommunityContractData> {
    ...
    this.contractPromise = this.readContract(
      communityTxId
    ) as Promise<CommunityContractData>;
  ...
  async getTipPercentageFromContract(): Promise<CommunityTipPercentage> {
    ...
    const arDriveCommTipFromSettings: [string, unknown] | undefined =
      contract.settings.find((setting) => setting[0] === "fee");
