...
import { dynamoAvailable } from "../utils/dynamoDbUtils";
                  ...
                  return nestedDataItemParallelLimit(async () => {
                    ...
                    await useAndCleanupReadable(
                      ...
                      async (rawNestedDataItemReadable: Readable) => {
                        ...
                        // If Dynamo is not available or this item is too large for it, back up to S3
                        // TODO: CAN REMOVE THIS ONCE ar.io NODES CAN USE DYNAMO OFFSETS TO FETCH FROM PARENT VIA ANY OTHER STORE
                        const dynamoIsAvailable = dynamoAvailable();
                        const itemFitsInDynamo =
                          contentLength <=
                          (await getConfigValue(
                            ConfigKeys.dynamoDataItemBytesThreshold
                          ));
                        const willCacheToDynamoDB =
                          dynamoIsAvailable && itemFitsInDynamo;
